...
async function requestAdminJson<TData>(path: string): Promise<AdminApiResult<TData>> {
  const apiBaseUrl = getApiBaseUrl();
  if (!apiBaseUrl) {
...
export async function uploadAdminShopifyImport(
  ...
  file: File,
): Promise<AdminApiResult<AdminImportJobDetail>> {
  const apiBaseUrl = getApiBaseUrl();
  if (!apiBaseUrl) {
    return { ok: false, reason: "api_url_missing" };
  }
  ...
  try {
    ...
    const response = await fetch(`${apiBaseUrl}/admin/imports/shopify/${importType}`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
      },
    ...
    const data = (await response.json()) as AdminImportJobDetail;
    return { ok: true, data };
  } catch {
    return { ok: false, reason: "fetch_failed" };
  }
